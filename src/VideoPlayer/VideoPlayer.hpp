...
class VIDEO_PLAYER_DLL VideoPlayer : public cocos2d::CCNodeRGBA {
  ...
  public:
     ...
     *
     * @param path Path to the video file (Could use
     * Mod::get()->getResourcesDir() / "file.mpg").
     * @param loop Whether or not playback should loop upon completion.
     * @return A new initialized video player
     */
    static VideoPlayer *create(std::filesystem::path const &path,
			       bool loop = false);

    /**
     * @brief Sets the content height of the video player, maintaining aspect
     * ratio
     *
    ...
    /**
     * @brief Sets the content width of the video player, maintaining aspect
     * ratio
     *
    ...
    /**
     * @brief Modifies the content size of the video player to fit a given size,
